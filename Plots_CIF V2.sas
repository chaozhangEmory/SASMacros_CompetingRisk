...
also, the number at risk at specified time-point with accumulated number of events of interest, 
...
%macro Plots_CIF(dsn=, grplist=, time_event=, censor=, eventcode=1, xaxisvalue=, yaxisvalue=, timelist=,units=,outpath=,filename=,debug=F);
...
QUIT;
     ...
     data Anno_AtRisk;
      ...
      run;
       ...
       title height=12pt font=Arial  "CIF Plot with Number AtRisk (Events Interest and Competing)" ;
      proc sgplot data= _est_plot  pad=(bottom=15% right=8%) sganno = Anno_AtRisk;
	      %if &yaxisvalue ~= %str() %then %do; yaxis values=(&yaxisvalue);%end;
		  %else %do;
          ...
          inset ("Gray's Test p="="%TRIM(&PVAL1)")/position=topleft border LABELALIGN=LEFT;
		  keylegend "CIF";
...
%Plots_CIF(dsn= bmt,
          ...
          censor=status,
          eventcode=1,
          xaxisvalue=0  500 1000 1500 2000 2500,
          yaxisvalue= , 
          timelist=100 500 800 ,
          units= days,
		  filename=ccqaqw,
		  outpath= &dir.\ ,
          debug=T)
